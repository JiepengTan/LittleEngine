...
{
	template<typename T>
	inline ImGuiDataType_ GUIUtil::GetDataType()
	{
	...
	template<typename T>
	inline std::string GUIUtil::GetFormat()
	{
	...
	template<typename T>
	inline void GUIUtil::DrawScalar( const std::string & p_name, T & p_data, float p_step, T p_min, T p_max)
	{
		static_assert(std::is_scalar<T>::value, "T must be a scalar");

		GUIUtil::CreateTitle( p_name);
		auto& widget = GUIUtil::CreateWidget<LittleEngine::UI::Widgets::Drags::DragSingleScalar<T>>(GetDataType<T>(), p_min, p_max, p_data, p_step, "", GetFormat<T>());
		auto& dispatcher = widget.AddPlugin<LittleEngine::UI::Plugins::DataDispatcher<T>>();
		dispatcher.RegisterReference(p_data);
	}

	template<typename T>
	inline void GUIUtil::DrawScalar( const std::string & p_name, std::function<T(void)> p_gatherer, std::function<void(T)> p_provider, float p_step, T p_min, T p_max)
	{
		static_assert(std::is_scalar<T>::value, "T must be a scalar");

		GUIUtil::CreateTitle( p_name);
		auto& widget = GUIUtil::CreateWidget<LittleEngine::UI::Widgets::Drags::DragSingleScalar<T>>(GetDataType<T>(), p_min, p_max, static_cast<T>(0), p_step, "", GetFormat<T>());
		auto& dispatcher = widget.AddPlugin<LittleEngine::UI::Plugins::DataDispatcher<T>>();
