...
void LittleEngine::Rendering::Buffers::UniformBuffer::BindBlockToShader(LittleEngine::Shader& p_shader, uint32_t p_uniformBlockLocation, uint32_t p_bindingPoint)
{
	glUniformBlockBinding(p_shader.id, p_uniformBlockLocation, p_bindingPoint);
}

void LittleEngine::Rendering::Buffers::UniformBuffer::BindBlockToShader(LittleEngine::Shader& p_shader, const std::string& p_name, uint32_t p_bindingPoint)
{
	glUniformBlockBinding(p_shader.id, GetBlockLocation(p_shader, p_name), p_bindingPoint);
}

uint32_t LittleEngine::Rendering::Buffers::UniformBuffer::GetBlockLocation(LittleEngine::Shader& p_shader, const std::string& p_name)
{
