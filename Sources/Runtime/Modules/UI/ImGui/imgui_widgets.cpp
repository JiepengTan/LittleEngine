...
bool ImGui::ImageButton(ImTextureID user_texture_id, const ImVec2& posOffset, const ImVec2& size, const ImVec2& uv0, const ImVec2& uv1, int frame_padding, const ImVec4& bg_col, const ImVec4& tint_col, int flags)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
    ...
    const ImVec2 padding = (frame_padding >= 0) ? ImVec2((float)frame_padding, (float)frame_padding) : style.FramePadding;
    auto cursorPos = posOffset;
    const ImRect bb(cursorPos, cursorPos + size + padding * 2);
    const ImRect image_bb(cursorPos + padding, cursorPos + padding + size);
    ItemSize(bb);
    if (!ItemAdd(bb, id))
        return false;
...
#include "Modules/UI/ImGui/imstb_textedit.h"
