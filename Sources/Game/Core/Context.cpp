...
LittleGame::Core::Context::Context() :
	...
	sceneManager(projectAssetsPath)
{
	...
	driver = std::make_unique<LittleEngine::Rendering::Context::Driver>(LittleEngine::Rendering::Settings::DriverSettings{ false });
	renderer = std::make_unique<LittleEngine::SceneRenderer>(*driver);

	renderer->SetCapability(LittleEngine::Rendering::Settings::ERenderingCapability::MULTISAMPLE, projectSettings.Get<bool>("multisampling"));
	...
	/* Physics engine */
	physicsEngine = std::make_unique<LittleEngine::Physics::Core::PhysicsEngine>(LittleEngine::Physics::Settings::PhysicsSettings{ {0.0f, projectSettings.Get<float>("gravity"), 0.0f } });
